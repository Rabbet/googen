...
defmodule Googly.CloudVision.Model.WebPage do
  @moduledoc """
  Metadata for web pages.

  ## Attributes

  *   `full_matching_images` (*type:* `list(Googly.CloudVision.Model.WebImage.t())`) - Fully matching images on the page. Can include resized copies of the query image.
  *   `page_title` (*type:* `String.t()`) - Title for the web page, may contain HTML markups.
  *   `partial_matching_images` (*type:* `list(Googly.CloudVision.Model.WebImage.t())`) - Partial matching images on the page. Those images are similar enough to share some key-point features. For example an original image will likely have partial matching for its crops.
